...
const Admin = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setFiles([]); setSelectedAlbumId(""); fetchAll();
    } catch (err) { setMessage("error:Upload failed!"); }
  ...
  const handleCreateAlbum = async () => {
    ...
    try {
      ...
      setNewAlbumName(""); setNewAlbumDesc(""); setShowAlbumForm(false); fetchAll();
    } catch (err) { setMessage("error:Failed!"); }
  };

  const handleDeleteAlbum = async (id) => {
    if (!window.confirm("Delete this album?")) return;
    try { await deleteAlbum(id); setMessage("success:Album deleted!"); fetchAll(); }
    catch (err) { setMessage("error:Failed!"); }
  };
  ...
  const handleMoveToAlbum = async (photoId, albumId) => {
    try { await moveToAlbum(photoId, albumId); setMessage("success:Moved!"); fetchAll(); }
    catch (err) { setMessage("error:Failed!"); }
  };
  ...
  const prevPhoto = (e) => {
    ...
    setSelected(photos[newIndex]); setSelectedIndex(newIndex);
  };
    ...
    <div style={{ ...styles.container, background: theme.bg, color: theme.text }}>
      <div style={{ ...styles.navbar, background: theme.navbar, borderBottom: "1px solid " + theme.border }}>
        ...
        <div style={styles.navRight}>
          ...
          <button style={styles.galleryBtn} onClick={() => navigate("/home")}>{isMobile ? "🏠" : "Gallery"}</button>
      ...
      {isMobile && (
        <div style={{ ...styles.mobileTabs, background: theme.navbar, borderBottom: "1px solid " + theme.border }}>
          {["upload", "photos", "albums"].map((tab) => (
            <button key={tab} style={{ ...styles.mobileTab, background: activeTab === tab ? "linear-gradient(135deg,#667eea,#764ba2)" : "transparent", color: activeTab === tab ? "white" : theme.subText }} onClick={() => setActiveTab(tab)}>
              {tab === "upload" ? "⬆️ Upload" : tab === "photos" ? "🖼️ Photos" : "📁 Albums"}
            </button>
          ))}
        </div>
      )}
      ...
      <div style={{ ...styles.content, padding: isMobile ? "12px" : "24px" }}>
        ...
        {activeTab === "upload" && (
          <div style={{ ...styles.uploadLayout, flexDirection: isMobile ? "column" : "row" }}>
            <div style={styles.uploadMain}>
              <div style={{ ...styles.uploadCard, background: theme.cardBg, border: "1px solid " + theme.border }}>
                <h2 style={{ ...styles.cardTitle, color: theme.text, fontSize: isMobile ? "16px" : "18px" }}>Upload Photos and Videos</h2>
                <div
                  style={{ ...styles.dropZone, borderColor: dragging ? "#667eea" : theme.border, background: dragging ? "rgba(102,126,234,0.08)" : theme.inputBg, padding: isMobile ? "24px 16px" : "40px 24px" }}
                ...
                >
                  ...
                  <p style={{ ...styles.dropTitle, color: theme.text, fontSize: isMobile ? "14px" : "16px" }}>Drop photos and videos here</p>
                  <p style={{ color: theme.subText, fontSize: "12px", margin: "4px 0" }}>or tap to browse</p>
                  <p style={{ color: theme.subText, fontSize: "11px", margin: 0 }}>JPG, PNG, GIF, MP4, MOV</p>
                    ...
                    <div style={styles.previewGrid}>
                      {files.map((f, i) => (
                        <div key={i} style={styles.previewItem}>
                          {f.type.startsWith("video") ? <div style={styles.videoPreview}>🎥</div> : <img src={URL.createObjectURL(f)} alt={f.name} style={styles.previewImg} />}
                          <p style={{ color: theme.subText, fontSize: "10px", margin: "4px 0 0", textAlign: "center" }}>{f.name.substring(0, 8)}...</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {uploading && (
                  <div style={styles.progressSection}>
                    <div style={{ ...styles.progressBar, background: theme.border }}><div style={{ ...styles.progressFill, width: progress + "%" }} /></div>
                ...
                <div style={{ ...styles.storageCard, background: theme.cardBg, border: "1px solid " + theme.border }}>
                  ...
                  <div style={styles.storageCircle}>
                    <svg width="120" height="120" viewBox="0 0 120 120">
                      ...
                      <circle cx="60" cy="60" r="50" fill="none" stroke="url(#grad)" strokeWidth="10" strokeDasharray={2 * Math.PI * 50} strokeDashoffset={2 * Math.PI * 50 * (1 - storagePercent / 100)} strokeLinecap="round" transform="rotate(-90 60 60)" />
                      <defs><linearGradient id="grad" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="#667eea" /><stop offset="100%" stopColor="#764ba2" /></linearGradient></defs>
                  ...
                  <div style={{ ...styles.storageBar, background: theme.border }}><div style={{ ...styles.storageFill, width: storagePercent + "%" }} /></div>
                  <div style={styles.storageStats}>
                    <div style={styles.statItem}><p style={{ color: theme.subText, fontSize: "11px", margin: "0 0 2px" }}>Photos</p><p style={{ color: theme.text, fontWeight: "700", margin: 0 }}>{photos.filter(p => p.mediaType !== "video").length}</p></div>
                    <div style={styles.statItem}><p style={{ color: theme.subText, fontSize: "11px", margin: "0 0 2px" }}>Videos</p><p style={{ color: theme.text, fontWeight: "700", margin: 0 }}>{photos.filter(p => p.mediaType === "video").length}</p></div>
                    <div style={styles.statItem}><p style={{ color: theme.subText, fontSize: "11px", margin: "0 0 2px" }}>Albums</p><p style={{ color: theme.text, fontWeight: "700", margin: 0 }}>{albums.length}</p></div>
                  </div>
                </div>
              </div>
            )}
            ...
            {isMobile && (
              <div style={{ ...styles.mobileStorage, background: theme.cardBg, border: "1px solid " + theme.border }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <p style={{ color: theme.text, fontWeight: "700", margin: 0, fontSize: "13px" }}>Storage: {storageUsedGB} GB / {storageLimitGB} GB</p>
                  <p style={{ color: "#667eea", fontWeight: "700", margin: 0, fontSize: "13px" }}>{storagePercent}%</p>
                </div>
                <div style={{ ...styles.storageBar, background: theme.border, marginTop: "8px" }}><div style={{ ...styles.storageFill, width: storagePercent + "%" }} /></div>
                <div style={{ display: "flex", gap: "16px", marginTop: "10px" }}>
                  <p style={{ color: theme.subText, fontSize: "12px", margin: 0 }}>📷 {photos.filter(p => p.mediaType !== "video").length} Photos</p>
                  <p style={{ color: theme.subText, fontSize: "12px", margin: 0 }}>🎥 {photos.filter(p => p.mediaType === "video").length} Videos</p>
                  <p style={{ color: theme.subText, fontSize: "12px", margin: 0 }}>📁 {albums.length} Albums</p>
                </div>
              </div>
            )}
          </div>
        ...
        {activeTab === "photos" && (
          <div>
            <div style={styles.sectionHeader}>
              <h2 style={{ ...styles.cardTitle, color: theme.text, fontSize: isMobile ? "16px" : "18px" }}>All Media ({photos.length})</h2>
            </div>
            {loading ? (
              <div style={{ ...styles.photoGrid, gridTemplateColumns: isMobile ? "repeat(3,1fr)" : "repeat(auto-fill,minmax(180px,1fr))" }}>
                {[...Array(6)].map((_, i) => <div key={i} style={{ ...styles.skeleton, background: theme.skeleton }} />)}
              </div>
            ) : photos.length === 0 ? (
              <div style={{ ...styles.empty, color: theme.subText }}><div style={styles.emptyIcon}>🖼️</div><p>No media uploaded yet!</p></div>
            ) : (
              <div style={{ ...styles.photoGrid, gridTemplateColumns: isMobile ? "repeat(3,1fr)" : "repeat(auto-fill,minmax(180px,1fr))" }}>
                {photos.map((photo, index) => (
                  <div key={photo.id} style={{ ...styles.photoCard, background: theme.cardBg }} onClick={() => openPhoto(photo, index)}>
                    {photo.mediaType === "video" ? (
                      <div style={styles.videoThumb}><video src={photo.imageUrl} style={styles.photoImg} /><div style={styles.playIcon}>▶</div></div>
                    ) : (
        ...
        {activeTab === "albums" && (
          <div>
            <div style={styles.sectionHeader}>
              <h2 style={{ ...styles.cardTitle, color: theme.text, fontSize: isMobile ? "16px" : "18px" }}>Albums ({albums.length})</h2>
            ...
            {albums.length === 0 ? (
              ...
              <div style={{ ...styles.albumGrid, gridTemplateColumns: isMobile ? "repeat(2,1fr)" : "repeat(auto-fill,minmax(200px,1fr))" }}>
                {albums.map((album) => (
                  <div key={album.id} style={{ ...styles.albumCard, background: theme.cardBg, border: "1px solid " + theme.border }}>
                    ...
                    <p style={{ color: theme.text, fontWeight: "700", fontSize: isMobile ? "13px" : "15px", margin: "0 0 4px" }}>{album.name}</p>
                    <p style={{ color: theme.subText, fontSize: "12px", margin: "0 0 12px" }}>{album.description || "No description"}</p>
                    <button style={styles.delAlbumBtn} onClick={() => handleDeleteAlbum(album.id)}>Delete</button>
                  </div>
                ))}
              </div>
            )}
        ...
        <div style={styles.lightbox} onClick={() => setSelected(null)}>
          <div style={styles.lightboxHeader}>
            ...
            <div style={styles.lightboxActions}>
              ...
              <a href={selected.imageUrl} target="_blank" rel="noreferrer" style={styles.lbDownload} onClick={(e) => e.stopPropagation()}>⬇️</a>
              <button style={styles.lbDelete} onClick={(e) => handleDelete(selected.id, e)}>🗑️</button>
...
const styles = {
  ...
  logoSection: { display: "flex", alignItems: "center", flexShrink: 0 },
  navTabs: { display: "flex", gap: "8px" },
  navTab: { padding: "7px 14px", borderRadius: "8px", cursor: "pointer", fontSize: "13px", fontWeight: "600", transition: "all 0.2s" },
  mobileTabs: { display: "flex", padding: "0", position: "sticky", top: "52px", zIndex: 99 },
  mobileTab: { flex: 1, padding: "10px 4px", border: "none", cursor: "pointer", fontSize: "12px", fontWeight: "600", transition: "all 0.2s" },
  ...
  galleryBtn: { padding: "7px 12px", background: "linear-gradient(135deg,#667eea,#764ba2)", color: "white", border: "none", borderRadius: "8px", fontWeight: "600", cursor: "pointer", fontSize: "13px" },
  logoutBtn: { padding: "7px 12px", borderRadius: "8px", fontWeight: "600", cursor: "pointer", fontSize: "12px" },
  content: { maxWidth: "1400px", margin: "0 auto" },
  msg: { padding: "12px 16px", borderRadius: "10px", marginBottom: "16px", fontWeight: "600", fontSize: "14px" },
  uploadLayout: { display: "flex", gap: "24px" },
  uploadMain: { flex: 1 },
  uploadSide: { width: "280px", flexShrink: 0 },
  uploadCard: { borderRadius: "16px", padding: "20px", marginBottom: "16px" },
  cardTitle: { fontWeight: "700", margin: "0 0 16px" },
  dropZone: { border: "2px dashed", borderRadius: "14px", textAlign: "center", cursor: "pointer", transition: "all 0.3s", marginBottom: "16px" },
  dropIconBg: { width: "56px", height: "56px", background: "linear-gradient(135deg,#667eea,#764ba2)", borderRadius: "50%", margin: "0 auto 12px", display: "flex", alignItems: "center", justifyContent: "center" },
  dropPlus: { color: "white", fontSize: "28px", fontWeight: "300", lineHeight: "1" },
  dropTitle: { fontWeight: "600", margin: "0 0 4px" },
  previewSection: { marginBottom: "14px" },
  previewHeader: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" },
  ...
  previewImg: { width: "64px", height: "64px", objectFit: "cover", borderRadius: "8px" },
  videoPreview: { width: "64px", height: "64px", background: "rgba(102,126,234,0.2)", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "20px" },
  albumSelect: { marginBottom: "14px" },
  ...
  uploadBtn: { width: "100%", padding: "14px", background: "linear-gradient(135deg,#667eea,#764ba2)", color: "white", border: "none", borderRadius: "12px", fontSize: "15px", fontWeight: "700", cursor: "pointer", boxShadow: "0 8px 24px rgba(102,126,234,0.3)" },
  ...
  mobileStorage: { borderRadius: "12px", padding: "14px", marginTop: "0" },
  ...
  playIcon: { position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", background: "rgba(0,0,0,0.7)", borderRadius: "50%", width: "36px", height: "36px", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontSize: "14px" },
  photoOverlay: { position: "absolute", top: 0, left: 0, right: 0, height: "140px", background: "linear-gradient(transparent 40%,rgba(0,0,0,0.7))", opacity: 1, display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "6px" },
  ...
  createAlbumBtn: { padding: "9px 16px", background: "linear-gradient(135deg,#667eea,#764ba2)", color: "white", border: "none", borderRadius: "8px", fontWeight: "600", cursor: "pointer", fontSize: "13px" },
  albumForm: { borderRadius: "12px", padding: "16px", marginBottom: "16px", display: "flex", flexDirection: "column", gap: "10px" },
  ...
  albumCard: { borderRadius: "12px", padding: "20px", textAlign: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.08)" },
  albumIconLg: { fontSize: "40px", marginBottom: "10px" },
